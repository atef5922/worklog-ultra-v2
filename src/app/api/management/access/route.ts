 ...
 try {
  ...
  const [users,departments,teams]=await Promise.all([
   db.user.findMany({where:search?{OR:[{name:{contains:search,mode:"insensitive"}},{email:{contains:search,mode:"insensitive"}}]}:{},
    ...
    select:{id:true,name:true,email:true,role:true,isActive:true,departmentId:true,teamId:true,managementEnabled:true,accessVersion:true,permissions:true,accessScopes:true}}),
...
export async function PUT(request: Request) {
 try {
  ...
  const result=await db.$transaction(async tx=>{
   ...
   if(['departments','teams'].includes(input.scopeType)){
    const ids=[...new Set(input.scopeIds)];
    const count=input.scopeType==='departments'?await tx.department.count({where:{id:{in:ids}}}):await tx.team.count({where:{id:{in:ids}}});
    if(!ids.length || count!==ids.length) throw new AccessError("Select valid departments or teams for this scope.",400);
   }
   ...
   const scopes=input.role==='employee'?[]:['departments','teams'].includes(input.scopeType)
    ? [...new Set(input.scopeIds)].map(id=>({scopeType:input.scopeType,departmentId:input.scopeType==='departments'?id:null,teamId:input.scopeType==='teams'?id:null,grantedBy:actor.id}))
    : [{scopeType:input.scopeType,departmentId:null,teamId:null,grantedBy:actor.id}];
