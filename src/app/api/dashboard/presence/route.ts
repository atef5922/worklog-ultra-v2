...
export async function POST(request:Request){try{
 ...
 const presence=await db.$transaction(async tx=>{
  ...
  {
   const attendance=await tx.attendanceRecord.findUnique({where:{userId_attendanceDate:{userId:user.id,attendanceDate:new Date(toDateOnly())}},include:{workSessions:{where:{endedAt:null}},breakSessions:{where:{endedAt:null}}}});
  ...
  const validMeeting=current?.meetingStartedAt&&activeSession&&current.meetingStartedAt>=activeSession.startedAt?current.meetingStartedAt:null;
