...
export default async function PlanningPage({params}:{params:Promise<{id:string}>}){
 ...
 const task=await db.dailyTask.findFirst({where:{id,...personalOrScopedTasks(actor,'tasks.update')},include:{updates:{orderBy:[{reportDate:'desc'},{updatedAt:'desc'}],take:1}}});if(!task)notFound();
