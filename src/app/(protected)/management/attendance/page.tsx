...
import { AttendanceExportMenu } from "@/components/management/attendance-export-menu";
  ...
  return <div className="flex min-h-0 min-w-0 flex-col gap-2 min-[900px]:h-full min-[900px]:overflow-hidden" data-fit-viewport>
    <header className="flex shrink-0 flex-wrap items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        {canExport && <AttendanceExportMenu
          summaryExcelHref={`/api/management/attendance/export?${reportQuery}&format=xlsx`}
          summaryPdfHref={`/api/management/attendance/export?${reportQuery}&format=pdf`}
          detailedExcelHref={`/api/management/attendance/export?${detailedReportQuery}&format=xlsx`}
          detailedPdfHref={`/api/management/attendance/export?${detailedReportQuery}&format=pdf`}
        />}
      </div>
    </header>
    <AttendanceAutoFilters>
      <label className={labelClass}>From<DateInput className={inputClass} name="from" defaultValue={data.from} required /></label>
      <label className={labelClass}>To<DateInput className={inputClass} name="to" defaultValue={data.to} required /></label>
    ...
    <section className="flex min-h-0 min-w-0 flex-1 flex-col rounded-xl border border-[var(--panel-border)] bg-[var(--panel)] p-2.5">
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-auto overscroll-contain max-[899px]:max-h-[55vh]"><table aria-label="Daily attendance records" className={`${styles.table} w-full min-w-[1050px] text-left text-xs`}>
        ...
        <tbody>{data.pageRows.map(row => <tr key={`${row.date}:${row.employeeId}`} className="align-top">
          <td className="whitespace-nowrap px-2 py-1.5">{formatDateInDhaka(row.date)}<small className="block text-[var(--muted-foreground)]">{row.dayKind !== "workday" ? row.dayKind === "off" ? "Off day" : "Leave" : "Workday"}</small></td>
          ...
          <td className="px-1 py-1"><AttendanceDetailsButton details={{
            date: formatDateInDhaka(row.date), employee: row.employeeName, department: row.department, team: row.team,
