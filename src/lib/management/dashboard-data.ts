...
import {attendanceDayKind} from './attendance-overview';
...
export async function dashboardData(actor:AccessActor,params:URLSearchParams){
 ...
 const options=await db.user.findMany({where:{AND:[employeeScope(actor,'employees.view'),{isActive:true}]},orderBy:[{name:'asc'},{id:'asc'}],select:{id:true,name:true,createdAt:true,department:{select:{id:true,name:true}}},take:5001});
 ...
 const attendancePeriod=['today','yesterday','last7'].includes(params.get('attendancePeriod')??'')?params.get('attendancePeriod')!:'today';
 const summaryEnd=new Date(today);
 if(attendancePeriod==='yesterday')summaryEnd.setUTCDate(summaryEnd.getUTCDate()-1);
 const summaryDays=Array.from({length:attendancePeriod==='last7'?7:1},(_,index)=>{const day=new Date(summaryEnd);day.setUTCDate(day.getUTCDate()-index);return day.toISOString().slice(0,10);});
 const summaryStart=new Date(summaryDays.at(-1)!);
 const overrideKeys=['company',...new Set(filtered.flatMap(p=>p.department?['department:'+p.department.id]:[])),...ids.map(id=>'employee:'+id)];
 const [tasks,attendance,livePeople,summaryRecords,summaryOverrides]=await Promise.all([
  ...
  attendanceAccess?db.user.findMany({where:{id:{in:ids},AND:[employeeScope(actor,'attendance.view')]},select:{id:true,name:true,avatarUrl:true,presence:true,attendanceRecords:{where:{attendanceDate:new Date(today)},include:{workSessions:{where:{endedAt:null}},breakSessions:{where:{endedAt:null}}}},taskOwner:{where:{id:taskAccess?undefined:{in:[]},timerStates:{some:{reportDate:new Date(today),runningStartedAt:{not:null}}}},select:{id:true,taskTitle:true,timerStates:true,updates:{orderBy:[{reportDate:'desc'},{updatedAt:'desc'}],take:1}}}}}):[],
  attendanceAccess?db.attendanceRecord.findMany({where:{userId:{in:ids},user:employeeScope(actor,'attendance.view'),attendanceDate:{gte:summaryStart,lte:summaryEnd}},select:{userId:true,attendanceDate:true,workSessions:{orderBy:{startedAt:'asc'},select:{startedAt:true}}}}):[],
  attendanceAccess?db.attendanceDayOverride.findMany({where:{attendanceDate:{gte:summaryStart,lte:summaryEnd},subjectKey:{in:overrideKeys}},select:{attendanceDate:true,subjectKey:true,kind:true,reason:true}}):[],
 ]);
 ...
 const live=livePeople.map(person=>{
  const a=person.attendanceRecords.find(r=>toDateOnly(r.attendanceDate)===today),session=a?.workSessions.find(s=>!s.endedAt);
  const running=person.taskOwner.filter(t=>t.updates[0]?.status==='in_progress'&&hasRunningTaskTimer(t,now));
  return {id:person.id,name:person.name,avatar:person.avatarUrl,...presenceState({checkedIn:!!session,hasAttendance:!!a?.checkInAt,onBreak:!!a?.breakSessions.length,runningTasks:running.length,meetingStartedAt:person.presence?.meetingStartedAt??null,lastSeenAt:person.presence?.lastSeenAt??null},now),tasks:running.map(t=>t.taskTitle)};
 });
