...
vi.mock('server-only', () => ({}));
const m = vi.hoisted(() => ({users: vi.fn(), person: vi.fn(), tasks: vi.fn(), attendance: vi.fn(),count:vi.fn()}));
vi.mock('@/lib/db', () => ({db: {user: {count:m.count,findMany: m.users, findFirst: m.person}, dailyTask: {findMany: m.tasks}, attendanceRecord: {findMany: m.attendance}}}));
...
beforeEach(() => {vi.resetAllMocks(); vi.useFakeTimers(); vi.setSystemTime(at('2026-09-15T02:00:00')); m.count.mockResolvedValue(1); m.person.mockResolvedValue({id: 'employee'}); m.tasks.mockResolvedValue([]); m.attendance.mockResolvedValue([]);});
...
describe('Management overnight attendance and historical bounds', () => {
 ...
 it.each([{from:'2026-02-30',to:'2026-03-01'},{from:'2026-09-15',to:'2026-09-14'},{from:'2020-01-01',to:'2026-09-14'}])('rejects invalid or excessive date ranges: %o',range=>{
  ...
  it('live status considers open attendance from previous workdays', async () => {
    ...
    expect(m.users.mock.calls[1][0].select.attendanceRecords.where.OR).toContainEqual({workSessions: {some: {endedAt: null}}});
    expect(result.live[0].state).toBe('Available');
  });
});
