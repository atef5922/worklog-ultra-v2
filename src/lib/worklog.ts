  ...
  const tasks = await db.dailyTask.findMany({
      where: {
        ...
        OR: [
          ...
          {
            // A carried task keeps its original DailyTask row. Work performed
            // today lives in today's DailyTaskUpdate, so reports must discover
            // the task through that date too—not only through planDate.
            updates: {
              some: {
                reportDate: {
                  gte: startOfDay(start),
      ...
      include: {
        ...
        updates: {
          ...
          // from later or earlier days into the selected period.
          where: {
            reportDate: {
              gte: startOfDay(start),
              lte: endOfDay(end),
