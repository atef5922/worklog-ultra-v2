...
export function ForgotPasswordForm({ variant = "default" }: { variant?: "default" | "minimal" }) {
  ...
  async function onSubmit(formData: FormData) {
    ...
    const result = await parseApiResponse<{ message: string; code?: string }>(response, "Password reset request failed.");
    setLoading(false);
      ...
      <div className={cn("space-y-5", isMinimal && "space-y-4")}>
        ...
        >
          <div className="flex items-start gap-3">
            <KeyRound className={cn("mt-0.5 h-5 w-5 text-cyan-300", isMinimal && "text-[#173b71]")} />
            <div>
              <p className={cn("font-semibold text-white", isMinimal && "text-slate-900")}>Your reset code</p>
              <p className="mt-1">
                No email was sent. Use this code for{" "}
                <span className={cn("font-semibold text-white", isMinimal && "text-slate-900")}>{submittedEmail}</span>:
              </p>
              {resetCode ? <p className="mt-3 text-center text-2xl font-bold tracking-[0.3em]">{resetCode}</p> : null}
              <p className="mt-2">The code will expire in 30 minutes.</p>
            </div>
          </div>
        </div>
        <div className={cn("rounded-2xl border border-[var(--panel-border)] bg-[var(--panel-muted)] p-4", isMinimal && "rounded-lg border border-slate-700/45 bg-white/18 p-4")}>
          {showInlineReset ? <ResetPasswordForm presetEmail={submittedEmail} presetCode={resetCode ?? undefined} variant={variant} /> : null}
        </div>
        <Button
          className={cn(
            "w-full",
  ...
  return (
    <form action={onSubmit} className={cn("space-y-5", isMinimal && "space-y-4")}>
      ...
      <Button
        ...
        type="submit"
      >
        {loading ? "Creating..." : "Show Reset Code"}
      </Button>
      <p className={cn("text-sm text-[var(--muted-foreground)]", isMinimal && "text-[13px] text-slate-700")}>
        <Link href="/auth/login" className={cn("inline-flex items-center gap-2 text-white hover:text-cyan-300", isMinimal && "font-medium text-slate-900 hover:text-[#173b71]")}>
          <ArrowLeft className="h-4 w-4" />
