...
export function SignupForm({
  ...
  const [otpStage, setOtpStage] = useState<{ email: string; role: string; code: string } | null>(null);
  ...
  async function register(formData: FormData) {
    ...
    const result = await parseApiResponse<{ message: string; code: string }>(response, "Registration request failed.");
    setLoading(false);
    ...
    setOtpStage({ email: payload.email, role, code: result.code });
  }
      ...
      <form action={verify} className={cn("space-y-5", isMinimal && "space-y-4")}>
        ...
        >
          ...
          <span className={cn(isMinimal ? "font-semibold text-slate-900" : "text-white")}>
            {otpStage.email}
          </span>. No email was sent.
          <div className={cn("mt-3 rounded-xl bg-slate-950/20 px-4 py-3 text-center", isMinimal && "rounded-md bg-white/35")}>
            <p className="text-xs font-semibold uppercase tracking-[0.12em]">Your verification code</p>
            <p className="mt-1 text-2xl font-bold tracking-[0.3em] text-[var(--foreground)]">{otpStage.code}</p>
          </div>
        </div>
  ...
  return (
    <form action={register} className={cn("space-y-5", isMinimal && "space-y-4")}>
      ...
      <Button
        ...
        type="submit"
      >
        {loading ? "Creating verification..." : "Create Account"}
      </Button>
      <Link
        className={cn(
          "inline-flex h-11 w-full items-center justify-center rounded-xl border border-[var(--panel-border)] bg-[var(--panel-alt)] px-4 text-sm font-semibold text-[var(--foreground)] transition-colors hover:bg-[#2a3c55]",
